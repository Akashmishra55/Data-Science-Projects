...
class DataTransformation:
    ...
    def get_data_transformation(self):
        ...
        try:
            ...
            numerical_cols = ['amenities', 'accommodates', 'bathrooms', 'latitude', 'longitude', 'host_response_rate',
                              'number_of_reviews', 'review_scores_rating', 'bedrooms', 'beds']
            categorical_cols = ['property_type', 'room_type', 'bed_type', 'cancellation_policy', 'cleaning_fee', 'city',
                                'host_identity_verified', 'instant_bookable', 'host_has_profile_pic']

            # Define which columns should be ordinal-encoded and which should be scaled
            property_type_cat = ['Apartment', 'House', 'Condominium', 'Townhouse', 'Loft', 'Other', 'Guesthouse',
                                 'Bed & Breakfast', 'Bungalow', 'Villa', 'Dorm', 'Guest suite', 'Camper/RV',
                                 'Timeshare', 'Cabin', 'In-law', 'Hostel', 'Boutique hotel', 'Boat',
                                 'Serviced apartment', 'Tent', 'Castle', 'Vacation home', 'Yurt', 'Hut', 'Treehouse',
                                 'Chalet', 'Earth House', 'Tipi', 'Train', 'Cave', 'Casa particular', 'Parking Space',
                                 'Lighthouse', 'Island']
            ...
            cat_pipeline = Pipeline(
                steps=[
                    ...
                    ('ordinalencoder', OrdinalEncoder(
                        categories=[property_type_cat, room_type_cat, bed_type_cat, cancellation_policy_cat,
                                    cleaning_fee_cat, city_cat, host_has_profile_pic_cat, host_identity_verified_cat,
    ...
    def initialize_data_transformation(self, train_path, test_path):
        try:
            ...
            train_df['host_response_rate'] = train_df['host_response_rate'][
                train_df['host_response_rate'].notna()].str.replace('%', '').astype(int)

            # Convert only non-null values in the test_df
            test_df['host_response_rate'] = test_df['host_response_rate'][
                test_df['host_response_rate'].notna()].str.replace('%', '').astype(int)

            logging.info("Host Response Rate converted to int")

            target_column_name = 'log_price'
            drop_columns = [target_column_name, 'id', "name", "description", "first_review", "host_since",
            ...
            train_arr = np.concatenate([input_feature_train_arr, np.array(target_feature_train_df).reshape(-1, 1)],
                                       axis=1)
